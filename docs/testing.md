...
| IDOR / Postgres-backed API tests     | `pnpm run test:server` when `DATABASE_URL` is available, or `TEST_DATABASE_URL=postgres://… pnpm run test:server` to force a dedicated DB — runs `server/routes/api-idor.test.ts` (cross-tenant + **400** when exercise category ≠ workout **`workoutType`** on **`POST …/sets`**) |
...
- **Ownership:** Postgres IDOR suite runs when either **`TEST_DATABASE_URL`** (preferred in CI) or **`DATABASE_URL`** is available, including workout-type mismatch on log set.

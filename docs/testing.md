...
- **Client:** Vitest + Testing Library + MSW handlers for `/api/*` (including auth options and logout). **`client/src/lib/api/stats-api.test.ts`** and **`goals-api.test.ts`** lock query strings / methods for **`readVolumeSeries`**, **`readStatsSummary`**, and goals CRUD. **`client/src/lib/api/workouts-api.test.ts`** covers **`patchWorkout`** (finish / resume **`endedAt`**). MSW **`PATCH /api/workouts/:id`** keeps app tests aligned with workout updates. **`client/src/lib/exercise-catalog.test.ts`** — catalog subgroup keys and leaf filtering.
...
- **`e2e/smoke.spec.ts`** — happy path with demo or guest auth; cardio workout row asserts **Cardio** badge and filtered picker has no **Bench press** (skipped when the DB has no global cardio exercises — run **`database/seed-global-exercises-append.sql`** or reset + **`db:seed`**). CI uses a fresh Postgres service so the test runs there. Includes **finish workout from detail** (PATCH workout, **Completed** badge on detail, then home list shows **Open** for that workout).
- **E2E ports / rate limits:** **`pnpm run dev:e2e`** serves the client on **127.0.0.1:5188** with Vite **`--strictPort`** (no silent fallback to another port) and sets **`E2E_RELAX_RATE_LIMIT`** so the API skips `/api` rate limiters. Free **5188** and **8080** before **`pnpm run test:e2e`** (e.g. **`pnpm run dev:clean`**) if a stale dev server is bound — otherwise Playwright can target the wrong app and you may see confusing **429** responses. See **`docs/troubleshooting.md`**.
